    ...
    public class MyPayController : Controller
    {
        ...
        public async Task<IActionResult> PaymentReturn([FromForm] MyPayReturnModel returnModel)
        {
            ...
            try
            {
                ...
                MyPayProcessor aMyPayProcessor = new MyPayProcessor(); // 注意：這裡需要根據實際 DI 設定調整

                // 1. 驗證 hash 值
                // hash 是金流平台提供的資料完整性驗證碼，用於確保回傳資料未被篡改
                // 透過比對我們計算的 hash 值與金流平台提供的 hash 值來驗證資料真實性
                if (!aMyPayProcessor.VerifyMyPayHash(returnModel))
                {
                ...
                // 2. 處理回傳資訊並更新系統
                bool success = await aMyPayProcessor.ProcessMyPayReturn(returnModel);
