...
namespace ChurchReport.Controllers
{
    ...
    public class MyPayController : Controller
    {
        ...
        private ToolUtilityClass ToolUtility { get; set; }
        ...
        public IActionResult PaymentNotify([FromForm] MyPayReturnModel returnModel)
        {
            // ========================================
            // 🆕 最早期診斷：確認方法被呼叫與記錄執行環境
            // 在此加入 Trace 及附加診斷資訊，以判斷 logger 是否被跳過或符號未載入
            // ========================================
            System.Diagnostics.Trace.WriteLine("[Diag] PaymentNotify invoked");
            System.Diagnostics.Trace.WriteLine($"[Diag] PID: {System.Diagnostics.Process.GetCurrentProcess().Id}");
            try
            {
                System.Diagnostics.Trace.WriteLine($"[Diag] Controller assembly: {typeof(MyPayController).Assembly.Location}");
            }
            catch { }

            System.Diagnostics.Trace.WriteLine("[Diag] before _logger.LogInformation");
#if DEBUG
            // 在 Debug 模式下嘗試丟出中斷（若偵錯器附加則會停住）
            try { System.Diagnostics.Debugger.Break(); } catch { }
#endif

            _logger.LogInformation("========================================");
            _logger.LogInformation("[MyPay回傳] 開始處理金流回傳 - 時間: {Time}", DateTime.Now);
            _logger.LogInformation("[MyPay回傳] Request Method: {Method}, ContentType: {ContentType}",
                Request.Method, Request.ContentType);
            System.Diagnostics.Trace.WriteLine("[Diag] after _logger.LogInformation (initial)");

            // 記錄原始請求資料（診斷用）
            if (Request.HasFormContentType)
            {
                _logger.LogInformation("[MyPay回傳] 檢測到 Form 資料，欄位數量: {Count}", Request.Form.Count);
                foreach (var key in Request.Form.Keys)
                {
                    _logger.LogInformation("[MyPay回傳] Form[{Key}] = {Value}", key, Request.Form[key]);
                }
            }
            else
            {
                _logger.LogWarning("[MyPay回傳] ⚠️ 沒有檢測到 Form 資料，ContentType: {ContentType}", Request.ContentType);
            }

            _logger.LogInformation("[MyPay回傳] 收到金流回傳，OrderID: {OrderId}, UID: {Uid}, PRC: {Prc}",
                returnModel?.order_id, returnModel?.uid, returnModel?.prc);

            try
            {
                ToolUtility = new ToolUtilityClass("DYNAMICS365", "sunnyvalech");
